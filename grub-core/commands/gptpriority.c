                      ...
                      int argc, char **args)
{
  ...
  // Neverware: the original upstream patch has 0xfULL, but this
  // causes a warning if GRUB_CPU_SIZEOF_LONG is 8, because
  // PRIuGRUB_UINT64_T then expects an unsigned long, not an unsigned
  // long long. Specifying the exact type with a ULL or UL suffix
  // shouldn't actually be needed, so just remove the suffix and let
